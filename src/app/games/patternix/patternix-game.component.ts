...
export class PatternixGameComponent implements OnInit, OnDestroy {
  ...
  onKeydown(event: KeyboardEvent): void {
    const stage = this.game.stage();
    if (event.key === 'Enter' && stage === 'idle') {
      this.game.startSession();
    } else if (event.key === 'Escape' && (stage === 'playing' || stage === 'countdown')) {
      this.game.abortSession();
    }
  }
}
