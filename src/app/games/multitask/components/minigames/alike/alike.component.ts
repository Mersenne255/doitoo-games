...
export class AlikeComponent implements OnInit,OnDestroy,AfterViewInit{
  ...
  pick(i:number){if(this.done||this.solved()||this.failed()||this.si()!==null)return;this.si.set(i);const p=this.puzzle();if(!p)return;if(i===p.answerIndex){this.solved.set(true);this.correctCount.update((c: number)=>c+1);this.totalCount.update((c: number)=>c+1);this.clr();this.at=setTimeout(()=>this.next(),1000)}else this.end()}
  private end(){if(this.done)return;this.done=true;this.failed.set(true);this.totalCount.update((c: number)=>c+1);this.clr();const t=this.totalCount();this.completed.emit({slotIndex:this.slotIndex(),score:this.correctCount(),total:t,maxDifficulty:this.game.currentDifficulty(),details:{correct:this.correctCount(),incorrect:t-this.correctCount(),timedOut:0}})}
