...
function checkOverlappingCells(
  ...
  // Map from cell key to the first segment ID that occupies it
  const cellOwner = new Map<string, string>();

  // Collect endpoint node IDs (shared endpoints are allowed)
  ...
  for (const path of paths) {
    ...
    for (const cell of cells) {
      ...
      if (existing !== undefined && existing !== path.id) {
        // Allow shared endpoints (junction/station nodes where segments meet)
        if (endpointKeys.has(key)) {
          // Check if this cell is an endpoint of BOTH segments
          ...
          if (isEndpointOfCurrent && isEndpointOfExisting) {
            continue; // Shared endpoint — allowed
          }
        }
        errors.push(
...
function checkReachability(layout: BoardLayout, errors: string[]): void {
  ...
  // Build adjacency: nodeId → set of reachable nodeIds
  ...
  // Check all stations are reachable
...
function checkSpatialCoverage(
  ...
  // Compute coverage relative to grid dimensions
  // The maximum possible spread is from cell (0,0) center to cell (cols-1, rows-1) center
  const cellWidth = screenWidth / gridSize.cols;
  const cellHeight = screenHeight / gridSize.rows;
