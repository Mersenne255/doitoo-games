...
/**
 * Matcher minigame — Speed Match style.
 *
 * Shows one card at a time. Player decides if current card shares
 * any active property with the previous card.
 *
 * Animation approach: NO timeouts. The card is always in the DOM.
 * We use a simple `cardKey` signal that increments on each new card.
 * The `@for` directive with `track cardKey` destroys the old element
 * and creates a new one, which naturally triggers the CSS animation.
 * The card is never position:absolute — it's always in normal flow.
 */
...
export class MatcherComponent implements OnInit, OnDestroy {
  ...
  readonly displayCard = signal<ShapeCard | null>(null);  // what's rendered
  ...
  // canAnswer: only when game is active, not locked, not waiting
  ...
  // ── Timer state (only interval, no timeouts for animation) ──
  ...
  /** Display the first card for 2 seconds, then advance */
  private showFirstCard(): void {
    ...
    // Use the timer bar to show the 2s countdown
  ...
  /** Generate next card and start the answer countdown */
  private advanceToNextCard(): void {
    ...
    // Generate new card: 50/50 match vs no-match
  ...
  /** Generic countdown: updates timerPercent, calls onExpired when done */
  ...
  private handleGameOver(): void {
    ...
    // Ensure the current card is displayed
